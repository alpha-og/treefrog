...
func SyncTeXViewHandler(store *storage.Store) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(fmt.Sprintf(`{"page":%d,"x":%f,"y":%f,"file":"%s","line":%d}`, result.Page, result.X, result.Y, result.File, result.Line)))
	}
}
